script_name("OnlineRpHelper")
script_authors("Don Orlandini")
script_description("Скрипт делается исключительно для сервера Online Rp")
...
if not doesFileExist(getGameDirectory().."/SAMPFUNCS.asi") then
	...
	ffi.C.MessageBoxA(0, sampfuncsNot, "OnlineRpHelper", 0x00000030 + 0x00010000) 
end
if #nofiles > 0 then
	ffi.C.ShowWindow(ffi.C.GetActiveWindow(), 6)
	ffi.C.MessageBoxA(0, errorText:format(table.concat(nofiles, "\n\t\t")), "OnlineRpHelper", 0x00000030 + 0x00010000) 
end
...
--os.remove(dirml.."/OnlineRpHelper/files/update.txt")
--os.remove(dirml.."/OnlineRpHelper/files/update.med")
...
chgName.rank = {u8"Интернат", u8"Фельдшер", u8"Участковый Врач", u8"Терапевт", u8"Проктоло", u8"Окулист", u8"Хирург", u8"Заведующий отделением", u8"Зам.Главного Врача", u8"Глав. Врач"}
...
local list_rank	= {u8"Интернат", u8"Фельдшер", u8"Участковый Врач", u8"Терапевт", u8"Проктоло", u8"Окулист", u8"Хирург", u8"Заведующий отделением", u8"Зам.Главного Врача", u8"Глав. Врач"}
--chat
...
function imgui.BeforeDrawFrame()
  if fa_font == nil then
    ...
    fa_font = imgui.GetIO().Fonts:AddFontFromFileTTF('moonloader/OnlineRpHelper/files/font-icon.ttf', 15.0, font_config, fa_glyph_ranges)
  end
end
...
function main()
	...
	if script.this.filename:find("%.luac") then
		os.rename(getWorkingDirectory().."\\OnlineRpHelper.luac", getWorkingDirectory().."\\OnlineRpHelper.lua") 
	end
--	repeat wait(100) until sampIsLocalPlayerSpawned()
	------------
	...
	------------
		...
		if not doesFileExist(dirml.."/OnlineRpHelper/files/logo-OnlineRpHelper.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение logo-OnlineRpHelper.png"); scr:unload() end
		if not  doesFileExist(dirml.."/OnlineRpHelper/files/discord-logo.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-logo.png") end
		if not  doesFileExist(dirml.."/OnlineRpHelper/files/discord-site.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-site.png") end
		if not  doesFileExist(dirml.."/OnlineRpHelper/files/discord-role.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-role.png") end
		if not  doesFileExist(dirml.."/OnlineRpHelper/files/discord-med.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-med.png") end
		if not  doesFileExist(dirml.."/OnlineRpHelper/files/discord-nick.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-nick.png") end
		logoMH = imgui.CreateTextureFromFile(dirml.."/OnlineRpHelper/files/logo-OnlineRpHelper.png") 
		logoDis = imgui.CreateTextureFromFile(dirml.."/OnlineRpHelper/files/discord-logo.png")
		
		--Проверка на существование папкок
		if not doesDirectoryExist(dirml.."/OnlineRpHelper/files/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки под файлы")
			createDirectory(dirml.."/OnlineRpHelper/files/")
		end
		if not doesDirectoryExist(dirml.."/OnlineRpHelper/Binder/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для биндера.")
			createDirectory(dirml.."/OnlineRpHelper/Binder/")
		end
		if not doesDirectoryExist(dirml.."/OnlineRpHelper/Шпаргалки/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для шпор")
			createDirectory(dirml.."/OnlineRpHelper/Шпаргалки/")
		end
		if not doesDirectoryExist(dirml.."/OnlineRpHelper/Департамент/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для новостей в департамент")
			createDirectory(dirml.."/OnlineRpHelper/Департамент/")
		end
		--Загрузка файла настроек
		if doesDirectoryExist(dirml.."/OnlineRpHelper/Департамент/") then
			getGovFile()
		end
		if doesFileExist(dirml.."/OnlineRpHelper/rp-medcard.txt") then
			local f = io.open(dirml.."/OnlineRpHelper/rp-medcard.txt")
...
/medcard #playerID #healID #timeID 1000]]  
			local f = io.open(dirml.."/OnlineRpHelper/rp-medcard.txt", "w")
			f:write(textrp) 
			f:close()
			buf_mcedit.v = u8(textrp)
		end
		if doesFileExist(dirml.."/OnlineRpHelper/MainSetting.med") then
		print("{82E28C}Чтение настроек...")
		local f = io.open(dirml.."/OnlineRpHelper/MainSetting.med")
			local setf = f:read("*a")
			f:close()
			...
			if res and type(set) == "table" then 
				...
				os.remove(dirml.."/OnlineRpHelper/MainSetting.med")
		...
		if not cb_imageDis.v then
			disSite = imgui.CreateTextureFromFile(dirml.."/OnlineRpHelper/files/discord-site.png")
			disRole = imgui.CreateTextureFromFile(dirml.."/OnlineRpHelper/files/discord-role.png")
			disMed = imgui.CreateTextureFromFile(dirml.."/OnlineRpHelper/files/discord-med.png")
			disNick = imgui.CreateTextureFromFile(dirml.."/OnlineRpHelper/files/discord-nick.png")
		end

	print("{82E28C}Чтение настроек команд...")
	if doesFileExist(dirml.."/OnlineRpHelper/cmdSetting.med") then
	--register cmd
		local f = io.open(dirml.."/OnlineRpHelper/cmdSetting.med")
		...
		if res and type(keys) == "table" then
			...
			os.remove(dirml.."/OnlineRpHelper/cmdSetting.med")
		end
	else
	...
	if doesFileExist(dirml.."/OnlineRpHelper/bindSetting.med") then
		local f = io.open(dirml.."/OnlineRpHelper/bindSetting.med")
		...
		if res and type(list) == "table" then
			...
			os.remove(dirml.."/OnlineRpHelper/bindSetting.med")
	...
	lockPlayerControl(false)
		...
		sampRegisterChatCommand("invite", funCMD.inv)
		sampRegisterChatCommand("uninvite", funCMD.unv)
		...
		sampRegisterChatCommand("kickout", funCMD.expel)
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Скрипт инициализирован.", 0xEE4848)
		repeat wait(100) until sampIsLocalPlayerSpawned()
		_, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
		myNick = sampGetPlayerNickname(myid)
		
		sampAddChatMessage(string.format("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Приветствую, %s. Для активации главного меню пропишите в чат {22E9E3}/mh.", sampGetPlayerNickname(myid):gsub("_"," ")), 0xEE4848)
		wait(200)
		if buf_nick.v == "" then 
			sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Похоже у тебя не настроена основная информация. ", 0xEE4848)
			sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Зайди в главном меню в раздел \"Настройки\" и настрой себе всё по \"фэн-шую\".", 0xEE4848)
		end
...
function imgui.OnDrawFrame()
	if mainWin.v then
		...
		imgui.Begin(fa.ICON_HEARTBEAT .. " OnlineRpHelper "..scr.version, mainWin, imgui.WindowFlags.NoResize);
			...
			for i,v in ipairs(chgName.rank) do
				...
				local f = io.open(dirml.."/OnlineRpHelper/MainSetting.med", "w")
				f:write(encodeJson(setting))
				f:flush()
				f:close()
				sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Настройки сохранены.", 0xEE4848)
				needSave = false
			end
			imgui.PopStyleColor(1)
			imgui.EndGroup()
						...
						if #spur.list ~= 20 then
							for i = 1, 20 do
								if not table.concat(spur.list, "|"):find("Шпаргалка '"..i.."'") then
									...
									spur.text.v = ""
									spurBig.v = false
									local f = io.open(dirml.."/OnlineRpHelper/Шпаргалки/Шпаргалка '"..i.."'.txt", "w")
									f:write("")
									f:flush()
									f:close()
									break
					...
					--	
						...
						--	imgui.SetCursorPosX(415)
							if imgui.Button(u8"Удалить", imgui.ImVec2(260, 20)) then
								if doesFileExist(dirml.."/OnlineRpHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") then
									os.remove(dirml.."/OnlineRpHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt")
								end
							...
							if imgui.Button(u8"Сохранить", imgui.ImVec2(260, 20)) then
								...
								if spur.name.v ~= "" then 
										...
										if doesFileExist(dirml.."/OnlineRpHelper/Шпаргалки/"..name..".txt") and spur.list[spur.select_spur] ~= name then
											bool = true
								...
								if not bool then
									local f = io.open(dirml.."/OnlineRpHelper/Шпаргалки/"..name..".txt", "w")
						...
						elseif not spurBig.v and (spur.select_spur >= 1 and spur.select_spur <= 20) then
							...
							if imgui.Button(u8"Редактировать", imgui.ImVec2(170, 20)) then
								spur.edit = true
								local f = io.open(dirml.."/OnlineRpHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt", "r")
					...
					if cmdBind[selected_cmd].rank <= num_rank.v+1 then
						...
						if imgui.Button(u8"Очистить активацию", imgui.ImVec2(140, 20)) then 
							...
							cmdBind[selected_cmd].key = {}
								local f = io.open(dirml.."/OnlineRpHelper/cmdSetting.med", "w")
						...
						imgui.Separator()
							for i,v in ipairs(binder.list) do
								if imgui.Selectable(u8(binder.list[i].name), binder.select_bind == i) then 
									...
									if doesFileExist(dirml.."/OnlineRpHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt") then
										local f = io.open(dirml.."/OnlineRpHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt", "r")
					...
					--	
						...
						--	imgui.SetCursorPosX(345)
							if imgui.Button(u8"Удалить", imgui.ImVec2(127, 20)) then
								...
								binder.key = {}
								if doesFileExist(dirml.."/OnlineRpHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt") then
									os.remove(dirml.."/OnlineRpHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt")
								end
								table.remove(binder.list, binder.select_bind) 
								local f = io.open(dirml.."/OnlineRpHelper/bindSetting.med", "w")
							...
							if imgui.Button(u8"Сохранить", imgui.ImVec2(127, 20)) then
								...
								if not bool then
									...
									local f = io.open(dirml.."/OnlineRpHelper/bindSetting.med", "w")
									local ftx = io.open(dirml.."/OnlineRpHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt", "w")
									f:write(saveJS)
									ftx:write(text)
									f:flush()
									ftx:flush()
			...
			if select_menu[6] then
				...
				imgui.BeginChild("help but", imgui.ImVec2(0,0), true)
					...
					if imgui.IsItemClicked(0) then
						print(shell32.ShellExecuteA(nil, 'open', dirml.."/OnlineRpHelper/Шпаргалки/", nil, nil, 1))
					end
					--
					imgui.Bullet(); imgui.SameLine()
			...
			if select_menu[7] then
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.Bullet()
					imgui.TextColoredRGB("Разработчик - {FFB700}Don Orlandini")
					imgui.Bullet()
					...
					imgui.TextColoredRGB("Благодарность blast.hk за методическую информацию и его пользователям в решении вопросов.")
						...
						imgui.SameLine()
						imgui.TextColoredRGB("Для связи: VK: {74BAF4}vk.com/legorchik") --или Discord - {74BAF4}egorcsh")
							if imgui.IsItemHovered() then imgui.SetTooltip(u8"Кликните ЛКМ, чтобы скопировать, или ПКМ, чтобы открыть в браузере")  end
							if imgui.IsItemClicked(0) then setClipboardText("https://vk.com/legorchik") end
							if imgui.IsItemClicked(1) then print(shell32.ShellExecuteA(nil, 'open', 'https://vk.com/legorchik', nil, nil, 1)) end
							imgui.SameLine()
						...
						imgui.TextColoredRGB("Желающие поддержать разработку денюшкой - ".."{68E15D}\"Открыть\"")
							...
							if imgui.IsItemClicked(0) then print(shell32.ShellExecuteA(nil, 'open', 'https://yoomoney.ru/to/4100118527890158', nil, nil, 1)) end
						...
						if imgui.Button(u8"Удалить скрипт", imgui.ImVec2(160, 20)) then 
							...
							sampAddChatMessage("", 0xEE4848)
							sampAddChatMessage("", 0xEE4848)
							sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Внимание! Подтвердите удаление командой {77DF63}/mh-delete.", 0xEE4848)
							mainWin.v = false
						--	sampShowDialog(1002, "{E94C4C}OnlineRpHelper | {8EE162}Удаление", remove, "Принял", "")
						end
				imgui.EndChild()
	...
	--	imgui.SetWindowFontScale(1.1)
		if spur.edit then
				...
				if imgui.Button(u8"Удалить", imgui.ImVec2(357, 20)) then
					...
					table.remove(spur.list, spur.select_spur) 
					spur.select_spur = -1
					if doesFileExist(dirml.."/OnlineRpHelper/Шпаргалки/"..u8:decode(spur.select_spur)..".txt") then
						os.remove(dirml.."/OnlineRpHelper/Шпаргалки/"..u8:decode(spur.select_spur)..".txt")
					end
					spur.name.v = ""
					spurBig.v = false
					spur.edit = false
			...
			imgui.BeginChild("spur spec", imgui.ImVec2(1081, 730), true)
				if doesFileExist(dirml.."/OnlineRpHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") then
					for line in io.lines(dirml.."/OnlineRpHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") do
			...
			imgui.BeginGroup()
				...
				if imgui.Button(u8"Начать", imgui.ImVec2(60, 20)) then
					if sobes.selID.v ~= "" then
						...
						sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Укажите id игрока для начала собеседования.", 0xEE4848)
					end
				end
				...
				imgui.EndChild()
					...
					elseif dep.bool[7] then
						...
						imgui.Spacing()
							...
							if imgui.IsItemClicked(0) then
								print(shell32.ShellExecuteA(nil, 'open', dirml.."/OnlineRpHelper/Департамент/", nil, nil, 1))
							end
						imgui.Spacing()
						imgui.Spacing()
						if imgui.Button(u8"Подать", imgui.ImVec2(225, 30)) then
							...
							-- 	sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Gov Новости назначить можно только от 0 до 15 минут.", 0xEE4848)
							-- 	return
							-- end
							
								if doesFileExist(dirml.."/OnlineRpHelper/Департамент/"..u8:decode(dep.news[dep.newsN.v+1])..".txt") then
								--	print(u8:decode(dep.news[dep.newsN.v+1]))
									for line in io.lines(dirml.."/OnlineRpHelper/Департамент/"..u8:decode(dep.news[dep.newsN.v+1])..".txt") do
										sampSendChat(line)
										wait(1000)
									end
								end
			...
			if #updinfo < 5 then
				...
				if newversion == scr.version then
					...
					imgui.BeginChild("update log", imgui.ImVec2(0, 0), true)
						if doesFileExist(dirml.."/OnlineRpHelper/files/update.txt") then
							for line in io.lines(dirml.."/OnlineRpHelper/files/update.txt") do
...
function onHotKeyCMD(id, keys)
	if thread:status() == "dead" then
		...
		for k, v in pairs(cmdBind) do
			if sKeys == tostring(table.concat(v.key, " ")) then
				...
				elseif k == 19 then
					if resTarg then
						sampSetChatInputEnabled(true)
						sampSetChatInputText("/invite "..targID)
					else
						sampSetChatInputEnabled(true)
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: В данный момент проигрывается отыгровка.", 0xEE4848)
	end
end
...
local function playBind(tb)
	if not tb.debug.file or #tb.debug.close > 0 then
		if not tb.debug.file then
			sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Файл с текстом бинда не обнаружен. ", 0xEE4848)
		elseif #tb.debug.close > 0 then
			sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Диалог, начало которого является строка №"..tb.debug.close[#tb.debug.close]..", не закрыт тегом {dialogEnd}", 0xEE4848)
		end
		addOneOffSound(0, 0, 0, 1058)
		return false
	end
		...
		for k, v in pairs(binder.list) do
			if sKeys == tostring(table.concat(v.key, " ")) then
				thread = lua_thread.create(function()		
					local dir = dirml.."/OnlineRpHelper/Binder/bind-"..v.name..".txt"	
					local tb = {}
...
function getSpurFile()
	spur.list = {}
    local search, name = findFirstFile("moonloader/OnlineRpHelper/Шпаргалки/*.txt")
	while search do
...
lua_thread.create(function()
	if doesDirectoryExist(dirml.."/OnlineRpHelper/Департамент/") then
		if doesFileExist(dirml.."/OnlineRpHelper/Департамент/День открытых дверей.txt") or not doesFileExist(dirml.."/OnlineRpHelper/Департамент/День открытых дверей ЛСМЦ.txt") then
			os.remove(dirml.."/OnlineRpHelper/Департамент/День открытых дверей.txt")
			local f = io.open(dirml.."/OnlineRpHelper/Департамент/День открытых дверей ЛСМЦ.txt", "w")
			f:write(govls)
			f:flush()
			f:close()
			local f = io.open(dirml.."/OnlineRpHelper/Департамент/День открытых дверей СФМЦ.txt", "w")
			f:write(govsf)
			f:flush()
			f:close()
			local f = io.open(dirml.."/OnlineRpHelper/Департамент/День открытых дверей ЛВМЦ.txt", "w")
			f:write(govlv)
			f:flush()
			f:close()
		end
		dep.news = {}
		local search, name = findFirstFile("moonloader/OnlineRpHelper/Департамент/*.txt")
		while search do
			if not name then findClose(search) else
...
funCMD = {} 
function funCMD.del()
	sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Вы успешно удалили скрипт.", 0xEE4848)
	sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Выгрузка скрипта из игры...", 0xEE4848)
	os.remove(scr.path)
	scr:reload()
end
function funCMD.lec(id)
	...
	if not u8:decode(buf_nick.v):find("[а-яА-Я]+%s[а-яА-Я]+") then
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Подождите-ка, сначала нужно заполнить базовую информацию. {90E04E}/mh > Настройки > Основная информация", 0xEE4848)
		return
	end
	if id:find("%d+") then
		thread = lua_thread.create(function()
			if not isCharInModel(PLAYER_PED, 416) then
				...
				wait(1000)
					sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Нажмите на  {23E64A}Enter{FFFFFF} для продолжения.", 0xEE4848)
					addOneOffSound(0, 0, 0, 1058)
					local len = renderGetFontDrawTextLength(font, "{FFFFFF}[{67E56F}Enter{FFFFFF}] - Продолжить")
					while true do
						wait(0)
	...
	sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /hl [id игрока].", 0xEE4848)
	end
end
function funCMD.med(id)
	...
	thread = lua_thread.create(function()
		--[[
			...
			sampSendChat("Предоставьте пожалуйста Ваш паспорт для определения требований.")		
				...
				sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Нажмите на цифру верхней панели для выбора вида мед.услуги.", 0xEE4848)
				addOneOffSound(0, 0, 0, 1058)
				while true do
					wait(0)
			...
			while true do
				...
				sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Нажмите на цифру верхней панели после ответа пациента.", 0xEE4848)
				addOneOffSound(0, 0, 0, 1058)
				local diag = 0
				local time = 0
				while true do
		...
		local dir = dirml.."/OnlineRpHelper/rp-medcard.txt"	
		local tb = {}
		tb = strBinderTable(dir)
		tb.sleep = 1.85
		tb.vars["playerID"] = id
	...
	sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду: /mc [id игрока].", 0xEE4848)
	end
...
function funCMD.narko(id)
	if thread:status() ~= "dead" then 
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /narko [id игрока].", 0xEE4848)
		end
end
	...
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				...
				wait(500)
				sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Нажмите на цифру верхней цифровой панели равная количеству выдаваемых рецептов.", 0xEE4848)
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /rec [id игрока].", 0xEE4848)
		end
end
function funCMD.post(stat)
	...
	if not isCharInModel(PLAYER_PED, 416) then -- not
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Чтобы заступить на мобильный пост, Вам необходимо сначала сесть в карету.", 0xEE4848)
		addOneOffSound(0, 0, 0, 1058)
	else
		...
		elseif bool then
			if stat:find(".+") then
				...
				sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Укажите обстановку, например, /post Спокойно.", 0xEE4848)
			end
		end
	end
end
	...
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				...
				wait(1000)
					addOneOffSound(0, 0, 0, 1058)
					sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Паспорт игрока автоматически закроется, после чего продолжится диалог..", 0xEE4848)
					repeat wait(0) until sampIsDialogActive()
					while sampIsDialogActive() do
						local memory = require 'memory'
						memory.setint64(sampGetDialogInfoPtr()+40, bool and 1 or 0, true)
				...
				sampSendChat("/b Покажите татуировки с помощью команды /showtatu")
					sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Нажмите на  {23E64A}Enter{FFFFFF} для продолжения или {23E64A}Page Down{FFFFFF}, чтобы закончить диалог.", 0xEE4848)
					addOneOffSound(0, 0, 0, 1058)
					local len = renderGetFontDrawTextLength(font, "{FFFFFF}[{67E56F}Enter{FFFFFF}] - Продолжить")
					while true do
					wait(0)
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /tatu [id игрока].", 0xEE4848)
		end	
end
function funCMD.warn(text)
	...
	if num_rank.v+1 < 8 then
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Данная команда Вам недоступна. Поменяйте должность в настройках скрипта, если это требуется.", 0xEE4848)
		return
	end
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /+warn [id игрока] [причина].", 0xEE4848)
		end
end
function funCMD.uwarn(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /-warn [id игрока].", 0xEE4848)
		end
end
function funCMD.inv(id)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /invite [id игрока].", 0xEE4848)
		end
end
function funCMD.unv(text)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /uninvite [id игрока] [причина].", 0xEE4848)
		end
end
function funCMD.mute(text)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /+mute [id игрока] [время в минутах] [причина].", 0xEE4848)
		end
end
function funCMD.umute(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /-mute [id игрока].", 0xEE4848)
		end
end
function funCMD.rank(text)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /gr [id игрока] [номер ранга].", 0xEE4848)
		end
end
function funCMD.osm()
	if thread:status() ~= "dead" then 
		...
		thread = lua_thread.create(function()
				sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Нажмите на {23E64A}Enter{FFFFFF}, если готовы начать осмотр.", 0xEE4848)
				addOneOffSound(0, 0, 0, 1058)
...
function funCMD.expel(par)
	...
	if par:find("(%d+)%s([а-яА-Я%a%s]+)") then
		...
		thread = lua_thread.create(function()
			...
			wait(1000)
			sampSendChat("/kickout "..id.." "..reas)
		end)
	else
	sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Используйте команду /kickout [id игрока] [Причина].", 0xEE4848)
	end
end
...
function funCMD.update()
	...
	local url = "https://github.com/ZacaDeveloper/OnlineRpHelper/blob/main/OnlineRpHelper.luac?raw=true"
	...
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				...
				addOneOffSound(0, 0, 0, 1058)
				sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Произошла ошибка при скачивании обновления. Активация резервого источника...", 0xEE4848)
		...
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
			...
			sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Скачивание завершено, перезагрузка библиотек...", 0xEE4848)
			reloadScripts()
			showCursor(false)
		end
	end)
	...
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				...
				setClipboardText("https://github.com/ZacaDeveloper/OnlineRpHelper/blob/main/OnlineRpHelper.luac?raw=true")
				updWin.v = false
			end
		end
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
	...
	sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Проверяем наличие обновлений...", 0xEE4848)
		...
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
				wait(1000)
				if doesFileExist(dirml.."/OnlineRpHelper/files/update.med") then
					local f = io.open(dirml.."/OnlineRpHelper/files/update.med", "r")
					...
					urlupd = upd.url
						if upd.version == scr.version then
							sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: Всё отлично, Вы используете самую новую версию скрипта.", 0xEE4848)
						else
							sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: {4EEB40}Имеется обновление.{FFFFFF} Напиши {22E9E3}/update{FFFFFF} для получения информации.", 0xEE4848)
							wait(5000)
							updWin.v = true
						end
					end
		...
		local dir = dirml.."/OnlineRpHelper/files/update.txt"
		local url = "https://raw.githubusercontent.com/ZacaDeveloper/OnlineRpHelper/main/update.txt"
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
					wait(1000)
					if doesFileExist(dirml.."/OnlineRpHelper/files/update.txt") then
					local f = io.open(dirml.."/OnlineRpHelper/files/update.txt", "r")
					updinfo = f:read("*a")
					f:close()
					end
				end)
...
function hook.onSendDialogResponse(id, but, list)
	if sampGetDialogCaption() == ">{FFB300}Посты" then
		if but == 1 then
			...
			sampAddChatMessage("{FFFFFF}[{EE4848}OnlineRpHelper{FFFFFF}]: На карте была выставлена метка места назначения.", 0xEE4848)
			addOneOffSound(0, 0, 0, 1058)
		elseif but == 0 then
		end
	end
